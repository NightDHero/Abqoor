...
const answerSheetInput = document.querySelector("#answer-sheet-input");
const pdfImportButton = document.querySelector("#pdf-import-button");
const pdfFileName = document.querySelector("#pdf-file-name");
const answerSheetFileName = document.querySelector("#answer-sheet-file-name");
const pdfStartQuestionNumberInput = document.querySelector("#pdf-start-question-number");
const questionNumberInput = document.querySelector("#question-number");
const logoutButton = document.querySelector("#logout-button");
const publishButton = document.querySelector("#publish-button");
const deleteAllButton = document.querySelector("#delete-all-button");
const undoDeleteButton = document.querySelector("#undo-delete-button");
const pdfConflictModal = document.querySelector("#pdf-conflict-modal");
const pdfConflictMessage = document.querySelector("#pdf-conflict-message");
const pdfConflictSinglePreview = document.querySelector("#pdf-conflict-single-preview");
const pdfConflictImage = document.querySelector("#pdf-conflict-image");
const pdfConflictNumber = document.querySelector("#pdf-conflict-number");
const pdfConflictStatus = document.querySelector("#pdf-conflict-status");
const pdfConflictCreatedAt = document.querySelector("#pdf-conflict-created-at");
const pdfConflictClipboardWrapper = document.querySelector("#pdf-conflict-clipboard-wrapper");
const pdfConflictClipboardText = document.querySelector("#pdf-conflict-clipboard-text");
const pdfConflictNote = document.querySelector("#pdf-conflict-note");
const pdfConflictSkipButton = document.querySelector("#pdf-conflict-skip-button");
const pdfConflictReplaceButton = document.querySelector("#pdf-conflict-replace-button");
const pdfConflictCancelButton = document.querySelector("#pdf-conflict-cancel-button");
...
function isQuestionConflictDetail(value) {
    if (!isRecord(value)) {
        return false;
    }
...
function syncQuestionActions(hasQuestions) {
    if (deleteAllButton) {
...
function updateAnswerSheetSelection() {
...
function formatNumberPreview(values, limit = 10) {
...
function getQuestionStatusLabel(question) {
    ...
    return `الإجابة: ${OPTION_LABELS[question.correct_option]}`;
}
...
function closePdfConflictModal() {
    pdfConflictModal?.classList.add("hidden");
}
async function openPdfConflictModal(detail) {
...
function buildPdfImportFormData(conflictStrategy) {
    const pdfFile = pdfInput?.files?.[0];
    const answerSheetFile = answerSheetInput?.files?.[0];
...
async function handlePdfImport(conflictStrategy) {
    if (!pdfImportButton) {
        return;
    }
    ...
    try {
        ...
        setFeedback(messageParts.join(" "), failedPages.length || draftQuestionNumbers.length || skippedExistingQuestionNumbers.length ? "neutral" : "success");
        await loadQuestions();
    }
    catch (error) {
...
async function handleSaveQuestionAnswer(questionId, button) {
    const card = button.closest(".question-card");
    const select = card?.querySelector(`.draft-answer-select[data-question-id="${questionId}"]`);
...
function buildUndoStateFromArchive(payload) {
...
async function handleDeleteQuestion(questionId, button) {
